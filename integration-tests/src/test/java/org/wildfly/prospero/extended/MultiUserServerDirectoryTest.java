...
public class MultiUserServerDirectoryTest extends WfCoreTestBase {
    ...
    private void subscribeServerToUpdateManifest(String containerManifestPath) throws IOException {
        ...
        channel.getRepositories().forEach(r->builder.addRepository(r.getId(), r.getUrl()));
        builder
                .setName(channel.getName())
                .setManifestUrl(new URL(containerManifestPath));
        FileUtils.writeStringToFile(channelDefinition.toFile(), ChannelMapper.toYaml(builder.build()));
    }
    ...
    private static void createUpdateManifest(File updatedManifest) throws IOException {
        ...
        // bump wildfly-cli
        final List<Stream> streams = originalManifest.getStreams().stream().map(s -> {
                    if (s.getGroupId().equals("org.wildfly.core") && s.getArtifactId().equals("wildfly-cli")) {
                        return new Stream(s.getGroupId(), s.getArtifactId(), WfCoreTestBase.UPGRADE_VERSION);
                    } else {
                        return s;
                    }
                })
                .collect(Collectors.toList());
        final ChannelManifest updateManifestDef = new ChannelManifest("update-manifest", null, null, streams);
        FileUtils.writeStringToFile(updatedManifest, ChannelManifestMapper.toYaml(updateManifestDef));
    }
