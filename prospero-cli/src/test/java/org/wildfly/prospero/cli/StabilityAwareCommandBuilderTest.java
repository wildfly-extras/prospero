...
public class StabilityAwareCommandBuilderTest {
    ...
    @Test
    public void testConcurrentStabilityAccess() throws InterruptedException {
        // Given - Multiple threads trying to build commands concurrently
        distributionInfoMock.when(DistributionInfo::getStability).thenReturn(Stability.Community);
        MainCommand mainCommand = new MainCommand(console, actionFactory);
        ...
        final List<CommandLine> results = Collections.synchronizedList(new ArrayList<>());

        // When - Multiple threads build commands simultaneously
        ...
        assertNull("No exceptions should occur during concurrent building", exception.get());
        assertEquals("All threads should complete", numThreads, results.size());

        // Verify all results are consistent
        for (CommandLine commandLine : results) {
            assertNotNull("CommandLine should be built", commandLine);
            List<String> optionNames = getOptionNames(commandLine.getCommandSpec());
            assertTrue("--stability option should be included at Community level",
                      optionNames.contains(CliConstants.STABILITY));
        }
    }
