...
public interface ProsperoLogger extends BasicLogger {
    ...
    @Message(id = 260, value = "Channel map data has been written to %s.")
    String channelMap(String name);

    @Message(id = 261, value = "An error occurred while writing channel data to the file %s.")
    @LogMessage(level = Logger.Level.ERROR)
    void unableToWriteChannelDataToFile(String name, @Cause IOException e);

    @Message(id = 262, value = "Unable to read the channel names from file %s.")
    @LogMessage(level = Logger.Level.ERROR)
